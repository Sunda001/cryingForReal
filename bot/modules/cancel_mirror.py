    ...
    if len(args) > 1:
        ...
        with download_dict_lock:
            ...
            try:
                ...
                pass
    if len(args) == 1:
        msg = f"Please reply to the <code>/{BotCommands.LeechCommand}</code> message which was used to start the download or send <code>/{BotCommands.CancelMirror} GID</code> to cancel it!"
        if mirror_message and mirror_message.message_id not in keys:
            if BotCommands.LeechCommand in mirror_message.text or \
               BotCommands.TarLeechCommand in mirror_message.text or \
               BotCommands.UnzipLeechCommand in mirror_message.text:
