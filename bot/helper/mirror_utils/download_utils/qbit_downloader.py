    ...
    def add_torrent(self, link, dire, listener, qbitsel):
        ...
        try:
            ...
            if BASE_URL is not None and qbitsel:
                ...
                buttons.buildbutton("Select Files", f"{BASE_URL}/PublicLeech/files/{self.ext_hash}")
    ...
    def update(self):
        ...
        try:
            ...
            if tor_info.state == "metaDL":
                self.stalled_time = time.time()
                if time.time() - self.meta_time >= 180: # timeout while downloading metadata
                    self.client.torrents_pause(torrent_hashes=self.ext_hash)
                    time.sleep(0.3)
                ...
                if time.time() - self.stalled_time >= 180: # timeout after downloading metadata
                    self.client.torrents_pause(torrent_hashes=self.ext_hash)
                    time.sleep(0.3)
                    self.listener.onDownloadError("Dead Torrent!")
                    self.client.torrents_delete(torrent_hashes=self.ext_hash)
